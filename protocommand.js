var protoCommand = {
    ...
    get: function(buffer) {
        ...
        try {
            ...
            if((buffer.toString('utf8', this.size, this.size+1) !== this.delimiter) || (buffer.length <= this.size)) return false;
    ...
    params: function(buffer){
        ...
        try {
            // If the command is not followed by a next indicator
            if((buffer.toString('utf8', this.size, this.size+1) !== this.delimiter) || (buffer.length <= this.size)) return false;
            return buffer.toString('utf8', this.size+1, buffer.length).split(this.delimiter);
        }catch(e) {
            return e;
        }
    },
    make: function(command) {
        ...
        try {
            return new Buffer(command.toString() + this.delimiter + Array.prototype.slice.apply(arguments).slice(1).join(this.delimiter), 'utf8');
        }catch(e) {
